"""
Original code from:
https://github.com/Xingyi-li-Vince/VI_PointConv/tree/main
"""
...
class PointConv(nn.Module):
    ...
    def forward(
        ...
        v_miu = (
            n_miu.unsqueeze(dim=2)
            - torch.matmul(n_miu.unsqueeze(dim=2), r_hat.permute(0, 1, 3, 2)).permute(
                0, 1, 3, 2
            )
            * r_hat
        )
        ...
        theta8 = torch.sum(
            r_miu
            * torch.cross(n_alpha, n_miu.unsqueeze(dim=2).repeat(1, 1, K, 1), dim=3),
            dim=3,
            keepdim=True,
        )
        ...
        new_feat = torch.matmul(
            input=new_feat.permute(0, 3, 1, 2), other=weights.permute(0, 3, 2, 1)
        ).view(B, M, -1)
    ...
    def _gather_nd(self, inputs, nn_idx):
        ...
        inputs_gather = (
            inputs.gather(dim=-1, index=nn_idx)
            .contiguous()
            .view(c, k, n2)
            .permute(2, 1, 0)
            .contiguous()
...
class PointConv(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(len(cfg.feat_dim) - 2, -1, -1):
            ...
            self.pointdeconv.append(
                PointConvTranspose(in_ch, out_ch, mlp, cfg, weightnet, mlp2)
            )
    ...
    def forward(
        self, feat, pointclouds, edges_self, edges_forward, edges_propagate, norms
    ):
        ...
        for i, pointconv in enumerate(self.pointconv):
            ...
            for res_block in self.pointconv_res[i]:
                ...
                sparse_feat = res_block(
                    sparse_xyz, sparse_feat, nei_inds, sparse_xyz_norm
                )
